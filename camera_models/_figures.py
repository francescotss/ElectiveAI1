...
class GenericPoint:
    ...
    def draw3d(
        ...
        # ax.scatter3D(*self.values, s=s, color="tab:blue")
        # ax.scatter3D(*x, s=s, color="tab:green")
        # ax.plot(*np.c_[C, self.values], color="tab:gray", alpha=0.5, ls="--")
        
        ax.scatter3D(*self.values, s=0.5, color="tab:blue")
        ax.scatter3D(*x, s=0.5, color="black")
        
        ax.plot(*np.c_[C, self.values], color="tab:gray", alpha=0.5, linewidth=0.2)
        if self.name is not None:
